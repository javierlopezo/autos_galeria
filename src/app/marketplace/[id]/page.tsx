...
export default function VehiculoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const contactWhatsApp = (phone: string, isAdmin = false) => {
    const mensaje = isAdmin
      ? `Hola Autos Galeria, necesito asistencia con el vehiculo ${vehiculo?.marca} ${vehiculo?.linea} placa ${vehiculo?.placa}`
      : `Hola, me interesa el vehiculo ${vehiculo?.marca} ${vehiculo?.linea} placa ${vehiculo?.placa} publicado en Autos Galeria`;
            ...
            <div className="card p-6 mb-6">
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  Solicitar asistencia de Autos Galeria
                </button>
                <a
                  href="https://gestionidentidad.presente.com.co/?redirect_uri=https://apps.presente.com.co/servicios-digitales-presente/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full py-3 px-4 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-lg transition-colors"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  SOLICITA TU CRÉDITO PRESENTE AQUÍ
                </a>
              </div>
            </div>
